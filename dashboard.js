...
async function showVaultUnlock() {
    ...
    // Check status
    try {
        const response = await fetch('/check_vault_status');
        const data = await response.json();

        if (data.needs_setup) {
            title.innerHTML = '🔐 Setup Vault Access';
            subtitle.innerHTML = 'Set a master password for your vault (Google Login users)';
            button.innerHTML = 'Set Password';
            button.onclick = setupVault;
        } else {
            title.innerHTML = '🔐 Unlock Vault';
            subtitle.innerHTML = 'Enter your master password to proceed';
            button.innerHTML = 'Unlock';
            button.onclick = verifyVault;
        }

        modal.classList.add('show');
    } catch (error) {
        showToast('Error checking vault status', 'error');
    }
}
...
async function setupVault() {
    ...
    try {
        const response = await fetch('/set_vault_password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.success) {
            ...
            verifyVault(); // Auto-verify after set
        } else {
            messageEl.innerHTML = `<div class="error-message">❌ ${data.message}</div>`;
        }
    } catch (error) {
        messageEl.innerHTML = '<div class="error-message">❌ Setup failed</div>';
    }
}
